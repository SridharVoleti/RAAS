...
import { COURSES, TESTIMONIALS, STATS } from '@/lib/courseData'
import type { Course } from '@/types'

const TOP_COURSES = [...COURSES].sort((a, b) => b.student_count - a.student_count).slice(0, 6)
...
export default function HomePage() {
  ...
  const displayCourses = activePath === 'raas'
    ? TOP_COURSES.filter(c => c.path_id === 1)
    : TOP_COURSES
  ...
  return (
    <div className="min-h-screen bg-brand-bg">
      ...
      {/* Main content: 65% courses / 35% testimonials */}
      <section className="max-w-7xl mx-auto px-4 py-10">
          ...
          <div className="lg:w-[35%]">
            <div className="lg:sticky lg:top-20">
              ...
              <div className="space-y-4 max-h-[600px] lg:max-h-[70vh] overflow-y-auto pr-1">
                {TESTIMONIALS.map(t2 => (
                  <TestimonialCard key={t2.id} testimonial={t2} lang={lang} />
                ))}
              </div>
            </div>
          </div>
...
function TestimonialCard({ testimonial, lang }: { testimonial: typeof TESTIMONIALS[0], lang: string }) {
  ...
  const initials = testimonial.reviewer_name.split(' ').map(w => w[0]).join('').slice(0, 2).toUpperCase()
  const course = COURSES.find(c => c.id === testimonial.course_id)
    ...
    <div className="bg-brand-card border border-brand-border rounded-xl p-4">
      <div className="flex items-center gap-3 mb-3">
        ...
        <div>
          ...
          {course && <p className="text-brand-gold-muted text-xs">{course.title_en}</p>}
        </div>
